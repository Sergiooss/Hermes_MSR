...
def generate_launch_description():
    moveit_config = MoveItConfigsBuilder("rover", package_name="hermes_moveit_config").to_moveit_configs()
    ...
    pkg_path = get_package_share_directory('hermes_description')
    model_path += join(pkg_path, 'models')
    resource_path += pathsep + model_path
    ...
    model_path = get_model_paths(['hermes_description'])

    robot_description_launcher = IncludeLaunchDescription(
        PathJoinSubstitution([
            FindPackageShare('hermes_moveit_config'),
            'launch',
            'rsp.launch.py'
        ])
    )
    ...
    rviz_config_file = PathJoinSubstitution(
        [FindPackageShare('hermes_description'), 'rviz', 'robot.rviz']
    )
    ...
    twist_stamped = Node(
        ...
        remappings=[
            ('cmd_vel_out', '/hermes_base_control/cmd_vel'),
            ('cmd_vel_in', '/cmd_vel'),
        ]
    )
